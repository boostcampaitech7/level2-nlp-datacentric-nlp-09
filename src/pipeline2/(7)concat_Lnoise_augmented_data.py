...
"""
이 코드는 Label Noise 데이터와 증강된 데이터를 병합하는 코드입니다.
Label Noise 데이터는 Label에 노이즈가 있고, 증강된 데이터는 라벨이 없습니다. 두 데이터를 병합하여 다음 Pipeline에서 Label을 복원합니다.
...
df.to_csv(f'data/preprocessed/Lnoise_and_augmented.csv', index=False)