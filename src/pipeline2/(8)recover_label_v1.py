...
"""
recover_label_v1과 recover_label_v2에서는 Label Noise를 복구하는 과정을 수행합니다.
v1에서는 Text Noise가 복구된 데이터셋만을 사용하여 Label Noise를 복구하고, v2에서는 v1에서 복구한 데이터셋 전체를 사용하여 Label Noise를 복구합니다.

모델은 xlm-roberta-large 기반 "classla/multilingual-IPTC-news-topic-classifier" 모델을 fine-tuning하여 사용합니다.
이 모델은 Text Noise가 복구된 데이터셋(정상화된 데이터)을 학습하고 Label Noise 데이터와 증강된 데이터의 라벨을 복구합니다.
최종적으로 Text와 Label이 모두 복구된 데이터셋을 최종적으로 저장합니다.
...
6. 두 개의 데이터셋을 병합하여 최종 CSV 파일(`clean_data.csv`)로 저장합니다.
