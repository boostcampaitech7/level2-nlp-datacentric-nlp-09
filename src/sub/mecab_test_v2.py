...
# 명사가 없거나, 명사들이 모두 길이가 1인 경우만 필터링
df = df[df['text'].str.split().apply(lambda x: len(x) > 0 and any(len(word) > 1 for word in x))]

# 결과를 CSV 파일로 저장
df.to_csv('data/preprocessed/train_noise_v7.csv', index=False, encoding='utf-8-sig')
